(function($,win){
	...
	UserGuideWalkComp.prototype={
		...
		showButton : function(id,appendTo){
			...
			var btn = this.config.buttons[id], //拿到按钮数据
				appendTo = appendTo || "";
			;
			if(!btn)return;
			...
			if(id === 'nextUserGuide') {
				let time1 = 3
				$(appendTo).append($('<a />',{id : id , class:`${btn.css} disabledButton`, html: `${time1}s`}));
			  this.setTimeOutKey = setInterval(() => {
					time1 = time1 - 1
					...
					if(time1 < 1) {
						...
						clearInterval(that.setTimeOutKey)
						that.setTimeOutKey = null;
					}
				}, 1000);
			} else {
				$(appendTo).append($('<a />',{id : id , class:btn.css, html:btn.i18n})); //渲染页面
			}
		...
		showStep : function(){
			...
			if(userGuidePopup && userGuidePopup.type === "modal"){
       	 	...
       	 	this.showButton("nextUserGuide","#guideInnerWrapper"); //显示下一步按钮
